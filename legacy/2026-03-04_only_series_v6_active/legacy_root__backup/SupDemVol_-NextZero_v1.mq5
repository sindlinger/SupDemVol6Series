...
input bool     InpHabilitarCompactacaoOverflow = true; // Compactação quando ativos > alvo
...
input int      InpMargemCompactacaoSuave = 1;      // Compacta só quando ativos > alvo + margem
...
input double   InpPesoScoreProximidade = 0.35;      // Peso no score de compactação
input double   InpPesoScoreDistanciaPreco = 0.25;   // Peso no score de compactação
input double   InpPesoScoreBaixoVolume = 0.25;      // Peso no score de compactação
input double   InpPesoScoreAntiguidade = 0.15;      // Peso no score de compactação
...
input int      InpFaixaEstreitaBucketPontos = 10;   // Faixa em pontos para classificar tudo no bucket central
      ...
      if(g_pivosInicializados && barraNova && rates_total >= 2) {
         ...
         if(idxFechada >= 0) {
            ...
            if(totalZonasTocadas > 1 &&
               ...
               Print("MERGE[mesma-barra][SKIP]: bloqueado por regra suave (evento já consumido, modo remoção off ou zonas dentro do limite).");
            }
            ...
            // Overflow suave: compacta só quando passar do limite suave (alvo + margem).
