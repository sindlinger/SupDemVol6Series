...
public:
   ...
   bool FundirZonaMaisFracaParaAbrirSlot(const ENUM_LINE_TYPE tipoPreferido, const double precoAtualRef) {
      ...
      g_pivos[idxVizinha].volumeBuy = viz.volumeBuy + fraca.volumeBuy;
      g_pivos[idxVizinha].volumeSell = viz.volumeSell + fraca.volumeSell;
      RecalcularVolumeTotalZona(idxVizinha);
   ...
   bool MergeQuandoCriacaoProxima(const int idxZonaProxima,
      ...
      if(conflitoSinal &&
         InpModoConflitoSinalEnriquecimento == CONFLITO_SINAL_SUBTRAIR) {
   ...
   bool MesclarZonasPorIndices(const int idxNova, const int idxVizinha, const double precoAtualRef) {
      ...
      g_pivos[idxVizinha].preco = (tipoNovo == LINE_TOP) ? novoSup : novoInf;
      g_pivos[idxVizinha].corAtual = ObterCorZona(tipoNovo);
      g_pivos[idxVizinha].volumeBuy = viz.volumeBuy + nova.volumeBuy;
      g_pivos[idxVizinha].volumeSell = viz.volumeSell + nova.volumeSell;
      RecalcularVolumeTotalZona(idxVizinha);
   ...
   bool MesclarZonasMesmoBarraMesmoTipo(const int idxBase,
      ...
      g_pivos[idxBaseAtivo].volumeBuy = base.volumeBuy + outra.volumeBuy;
      g_pivos[idxBaseAtivo].volumeSell = base.volumeSell + outra.volumeSell;
      RecalcularVolumeTotalZona(idxBaseAtivo);
   ...
   int ConsolidarZonasMuitoProximas(const double distanciaMerge, const double precoAtualRef) {
      ...
      g_pivos[idxKeep].volumeBuy = keep.volumeBuy + drop.volumeBuy;
      g_pivos[idxKeep].volumeSell = keep.volumeSell + drop.volumeSell;
      RecalcularVolumeTotalZona(idxKeep);
